...
from ._utils import _seq2arr, EuclidData
...
def _create_tabs(
    ...
    style_html = """
        <style>
            input.euclidgrid-tab-%(0)s {
                display: none;
            }
            input.euclidgrid-tab-%(0)s + label.euclidgrid-tab-label-%(0)s {
            ...
            input.euclidgrid-tab-%(0)s:checked + label.euclidgrid-tab-label-%(0)s {
                background: #FFF;
                border-bottom: 1px solid transparent;
            }
            input.euclidgrid-tab-%(0)s ~ .tab {
    ...
    for i, label in zip(tab_ids, tabs_order):
        ...
        html += '<input class="euclidgrid-tab-%s" type="radio" name="tabs-%s" id="tab%s"%s/>' % (tab_layout_id, tab_layout_id, i, ' checked ' if active_tab else '')  # NOQA E501
        html += '<label class="euclidgrid-tab-label-%s" for="tab%s">%s</label>' % (tab_layout_id, i, label)  # NOQA E501
        active_tab = False
    ...
    for i, label in zip(tab_ids, tabs_order):
        ...
        html += _create_imgs_grid(
            ...
            labels=labels(range(0, max_imgs_per_tab)),
...
def _create_tabs_euclid(
    ...
    ----------
    euclid_images : Sequence[EuclidData]
        List of EuclidData to be displayed in tabs layout.
        Currently supports images in the following formats:
        - str (local/remote URL)
    max_imgs_per_tab : int, optional
        How many samples from each label/class to display in a tab
    ...
    for i, label in zip(tab_ids, tabs_order):
        ...
        tab_imgs_mask = labels == label
        html += _create_imgs_grid_euclid(
            images=images[tab_imgs_mask],
            labels=labels[tab_imgs_mask],
            metadata=metadata[tab_imgs_mask],
            max_images=max_imgs_per_tab,
            img_width=img_width)
...
def _create_html_viewer(
    ...
    html_viewer = """
    <style>
        #euclidgrid-html-viewer-toggle-%(1)s {
            position: absolute;
            top: -9999px;
            left: -9999px;
            visibility: hidden;
        ...
        #euclidgrid-html-viewer-label-%(1)s { 
            position: relative;
        ...
        #euclidgrid-html-viewer-textarea-%(1)s {
        ...
        #euclidgrid-html-viewer-toggle-%(1)s:checked ~ #euclidgrid-html-viewer-textarea-%(1)s {
            height: 200px;
            display: block;
        }

        #euclidgrid-html-viewer-toggle-%(1)s:checked + #euclidgrid-html-viewer-label-%(1)s:after {
            content: "hide html";
            position: absolute;
            top: 0;
            left: 0;
    ...
    <div>
        <input type="checkbox" id="euclidgrid-html-viewer-toggle-%(1)s">
        <label id="euclidgrid-html-viewer-label-%(1)s" for="euclidgrid-html-viewer-toggle-%(1)s">show html</label>
        <textarea id="euclidgrid-html-viewer-textarea-%(1)s" readonly>
            %(0)s
        </textarea>
    </div>
...
def _create_img(
    ...
    <div class="euclidgrid-placeholder-div-%(0)s">
        <div id="euclidgrid-content-div-%(0)s-%(1)s" class="euclidgrid-content-div-%(0)s">
            ...
            %(3)s
            <a href="#!">
                <span class="euclidgrid-img-close"/>
            </a>
...
def _create_imgs_grid(
    ...
    html += '<div id="euclidgrid-imgs-container-div-%s">' % grid_style_uuid
    html += ''.join([
        _create_img(
...
def _create_imgs_grid_euclid(
    ...
    labels : Sequence[str or int]
        List of classes/labels for images to be grouped by.
        Must be same length as `images`.
    metadata : Sequence[str], optional
        List of custom strings to be drawn above each image.
        Must be same length as `images`, by default `None`.
    max_images : int, optional
        How many images to display (takes first N images).
    ...
    # create code with style definitions
    html, grid_style_uuid = _get_default_style(img_width, zoom_scale=5.0)
...
def _get_default_style(img_width: int, zoom_scale: float  = 2.5):
    ...
    html = """
        <style>
        #euclidgrid-imgs-container-div-%(0)s {
            width: 100%%;
            height: 100%%;
            margin: 0%%;
            overflow: auto;
        ...
        div.euclidgrid-placeholder-div-%(0)s {
            width: %(1)spx;
            display: inline-block;
            margin: 3px;
            position: relative;
        ...
        div.euclidgrid-content-div-%(0)s {
            width: %(1)spx;
            background: white;
            display: inline-block;
            vertical-align: top;
        ...
        div.euclidgrid-content-div-%(0)s span.euclidgrid-img-close {
            display: none;
        }

        div.euclidgrid-content-div-%(0)s span {
            width: 100%%;
            height: 100%%;
            position: absolute;
            top: 0;
        ...
        div.euclidgrid-content-div-%(0)s img {
            width: %(1)spx;
        }

        div.euclidgrid-content-div-%(0)s span.euclidgrid-img-close:hover {
            cursor: zoom-out;
        }
        
        div.euclidgrid-content-div-%(0)s span.euclidgrid-img-expand:hover {
            cursor: zoom-in;
        }

        div[id^=euclidgrid-content-div-%(0)s]:target {
            transform: scale(%(2)s);
        ...
        div[id^=euclidgrid-content-div-%(0)s]:target span.euclidgrid-img-close {
            display: block;
        }

        div[id^=euclidgrid-content-div-%(0)s]:target span.euclidgrid-img-expand {
            display: none;
        }
        </style>

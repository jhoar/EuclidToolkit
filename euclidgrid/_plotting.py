"""
This is the main module of euclidgrid package.
...
from ._html_helpers import (
    _display_html, _create_tabs, _create_imgs_grid, _create_tabs_euclid)
from ._utils import _get_class_representations, _seq2arr, EuclidData
...
def plot_class_tabs(
    ...
    This function (same as the whole euclidgrid package) is using IPython and HTML under the hood.
...
def plot_class_euclid(
        euclid_images: Sequence[EuclidData],
        max_imgs_per_tab: int = 30,
        img_width: int = 150,
        tabs_order: Sequence[str or int] = None):
    """
    Euclid specialisation of the plot_class_tabs
    Efficient and convenient way of displaying images in interactive tabs grouped by labels.
    For tabs ordering and filtering check out `tabs_order` param.
    This function (same as the whole euclidgrid package) is using IPython and HTML under the hood.
    ...
    Parameters
    ----------
    images : Sequence[EuclidData]
        List of images images and metadata to nbe displayed in tabs layout.
        Currently supports images in the following formats:
        - str (local/remote URL)
    max_imgs_per_tab : int, optional
        How many samples from each label/class to display in a tab
    ...
    # run html helper function to generate html content
    html = _create_tabs_euclid(
        euclid_images=euclid_images,
        max_imgs_per_tab=max_imgs_per_tab,
        img_width=img_width,
        tabs_order=tabs_order)
